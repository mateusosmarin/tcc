...
\chapter{Metodologia}
  ...
  A ferramenta foi desenvolvida em Python 3 \cite{python}, utilizando as bibliotecas GTK \cite{gtk} \cite{pygtk},
  para interface gráfica, NumPy \cite{numpy} para computação numérica eficiente, SciPy \cite{scipy} para projeto
  dos filtros e Matplotlib \cite{matplotlib} para visualização de dados.
...
\chapter{Resultados}
  ...
  \cite{scipy}, o que permite a filtragem de sinais e obtenção das diversas características do filtro, tais como
  sua função de transferência, resposta em frequência, resposta ao impulso e até mesmo a filtragem de um sinal
  arbitrário.
...
\chapter{Conclusão}
  ...
  bem como capacita ao aluno com uma ferramenta alternativa durante o aprendizado do tópico de processamento de
  sinais.
